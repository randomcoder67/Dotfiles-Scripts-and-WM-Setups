...
backupCurrent="$XDG_DATA_HOME/rc67/script_data/backup.txt"
...
_logCompletion() {
	LOG_DIR="$XDG_DATA_HOME/organisation_utils_rc67/log"
	if [ "${#COMP_WORDS[@]}" != "2" ]; then
		if [ "${COMP_WORDS[1]}" == "-d" ] && [ "${#COMP_WORDS[@]}" == "3" ]; then # If -d was chosen, then display the avalible years/months
			if [ ${#COMP_WORDS[2]} -ge 4 ]; then # If month already filled, start showing avalible days
				...
				[ -f "$LOG_DIR/${curLogMonth}.json" ] || return 0 # Check that month exists, if not, don't continue
				curMonthSuggestions=$(cat "$LOG_DIR/$curLogMonth.json" | jq -r 'keys[]' | sed "s/\(.*\)/$curLogMonth\1/g") # Get the days avalible in given month
				...
				COMPREPLY=( $(compgen -W "$(ls "$LOG_DIR" | grep -oE '[0-9]*')" -- "${COMP_WORDS[2]}") )
				[[ ${#COMPREPLY} == 4 ]] && compopt -o nospace
				return 0
			fi
