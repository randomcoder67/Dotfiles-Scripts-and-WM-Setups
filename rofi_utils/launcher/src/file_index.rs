...
pub fn reindex_files(conf: &Config, home_dir: &str, f: &mut BufWriter<File>) {
	...
	for mut dir in conf.check_locations.clone() {
		...
		files = do_read_dir(&dir, &conf.allowed_extensions, files, false);
	}
	...
	for mut dir in conf.check_locations_recursive.clone() {
		...
		files = do_read_dir(&dir, &conf.allowed_extensions, files, true);
	}
...
pub fn do_read_dir(dir: &str, allowed_extensions: &Vec<String>, mut files: Vec<(String, String)> ,recursive: bool) -> Vec<(String, String)> {
	let mut path = PathBuf::from(dir);
	files = read_recursive(&mut path, allowed_extensions, files, recursive);
	files
}

fn read_recursive(path: &mut PathBuf, allowed_extensions: &Vec<String>, mut files: Vec<(String, String)>, recursive: bool) -> Vec<(String, String)> {
	...
	for f in contents {
		...
		if file_type.is_dir() && recursive {
			...
			files = read_recursive(path, allowed_extensions, files, recursive);
			path.pop();
