...
const SOUND_EFFECT_PATH = "/rc67/resources/alarm.m4a"
...
func setTimer(secs int, silent bool) {
	...
	if !silent {
		soundEffectCommand := exec.Command("mpv", "--force-window=no", "--no-resume-playback", xdgDataHome + SOUND_EFFECT_PATH)
		soundEffectCommand.Start()
	}
	// Send system notification
	notificationCommand := exec.Command("notify-send", "-t", "9000", "Timer Completed")
